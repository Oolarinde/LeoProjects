...
def list_employees(
    ...
    if current_user.role in ("SUPER_ADMIN", "GROUP_ADMIN"):
        ...
        return employees_service.list_employees_multi(db, company_ids)
    return employees_service.list_employees(db, current_user.company_id)
...
def create_employee(
    ...
    if data.target_company_id and current_user.role in ("SUPER_ADMIN", "GROUP_ADMIN"):
        group_ids = get_group_company_ids_for_user(db, current_user)
    ...
    return employees_service.create_employee(
...
def update_employee(
    ...
    if current_user.role in ("SUPER_ADMIN", "GROUP_ADMIN"):
        group_ids = get_group_company_ids_for_user(db, current_user)
        return employees_service.update_employee_multi(
...
def delete_employee(
    ...
    employees_service.delete_employee(db, item_id, current_user.company_id)
