...
def get_my_group(
    ...
    group = group_service.get_user_group(db, current_user)
    if group is None:
...
def list_companies(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    return group_service.list_group_companies(db, group.id)
...
def add_company(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    member = group_service.add_company_to_group(
...
def create_subsidiary(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    company, member = group_service.create_subsidiary(
...
def update_company(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    company = group_service.update_company(
...
def set_parent_company(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    group_service.set_parent_company(db, group.id, company_id)
    db.commit()
...
def remove_company(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    group_service.remove_company_from_group(db, group.id, company_id)
    db.commit()
...
def add_user_to_company(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    membership = group_service.add_user_to_company(
...
def list_group_users(
    ...
    return group_service.list_group_users(db, group.id)
...
def update_user_access(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No group found")
    group_service.update_user_access(db, group.id, user_id, data.memberships)
    db.commit()
    return group_service.list_group_users(db, group.id)
...
def check_coa(
    ...
    members = group_service.list_group_companies(db, group.id)
    ...
    return group_service.check_coa_mismatches(db, group.id, company_id)
...
def fix_coa(
    ...
    if not any(m["company_id"] == str(company_id) for m in members):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Company not in your group")
    count = group_service.add_missing_accounts(db, group.id, company_id)
    db.commit()
...
def list_coa_template(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    return group_service.list_coa_template(db, group.id)
...
def create_coa_template_entry(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    entry = group_service.create_coa_template_entry(db, group.id, data.model_dump())
    db.commit()
    db.refresh(entry)
    return entry
...
def update_coa_template_entry(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    entry = group_service.update_coa_template_entry(db, group.id, entry_id, data.model_dump())
    db.commit()
    db.refresh(entry)
    return entry
...
def delete_coa_template_entry(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    group_service.delete_coa_template_entry(db, group.id, entry_id)
    db.commit()
...
def list_allocation_rules(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    return group_service.list_allocation_rules(db, group.id)
...
def create_allocation_rule(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    rule = group_service.create_allocation_rule(
    ...
    rules = group_service.list_allocation_rules(db, group.id)
...
def update_allocation_rule(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    group_service.update_allocation_rule(
...
def delete_allocation_rule(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    group_service.delete_allocation_rule(db, group.id, rule_id)
    db.commit()
...
def group_dashboard(
    ...
    if group is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No company group found")
    return group_service.get_group_dashboard(db, group.id, year)
